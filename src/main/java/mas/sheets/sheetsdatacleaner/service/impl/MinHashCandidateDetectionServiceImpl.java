...
public class MinHashCandidateDetectionServiceImpl implements MinHashCandidateDetectionService {
    ...
    private static final double LONG_ROW_THRESHOLD = 0.27;

    private static final int MIN_NGRAM_OVERLAP = 15;
    ...
    public Set<IndexPair<Integer, Integer>> generateCandidatePairs(List<String> rows) {
        int n = rows.size();
        MinHashData[] dataArr = new MinHashData[n];

        for (int i = 0; i < n; i++) {
            String row = rows.get(i);
            IntSet grams = collectTrigramHashes(row);
            int[] signature = buildSignature(grams);
            dataArr[i] = new MinHashData(signature, grams.toArray(), row.replace('|', ' ').length());
        }

        Map<Long, IntList> buckets = new HashMap<>(n * BAND_COUNT / 4);
        for (int idx = 0; idx < n; idx++) {
            int[] sig = dataArr[idx].signature;
        ...
        for (IntList list : buckets.values()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = i + 1; j < size; j++) {
                    int b = buf[j];
                    double thr = dynamicThreshold(dataArr[a].len, dataArr[b].len);
                    if (jaccardPasses(dataArr[a].ngrams, dataArr[b].ngrams, thr))
    ...
    private IntSet collectTrigramHashes(String raw) {
        ...
        for (int i = 0; i <= len - 3; i++) {
            int h = MurmurHash3.hash32x86(s.getBytes(StandardCharsets.UTF_8), i, 3, 0);
            set.add(h);
        }
        return set;
    }
    ...
    private boolean jaccardPasses(int[] a, int[] b, double thr) {
        ...
        if (inter < MIN_NGRAM_OVERLAP) return false;
        union -= inter;
    ...
    private static final class IntSet {
        ...
        void add(int v) {
            ...
            if (table[i] == Integer.MIN_VALUE) { table[i] = v; size++; }
        }
        int[] toArray() {
